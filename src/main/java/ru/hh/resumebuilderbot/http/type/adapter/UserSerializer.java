...
//        }
    ...
    private JsonElement serializeEducations(Set<Education> educations) {
        ...
        educationsJson.addProperty("level", HHUtils.convertEducationLevel(getCommonEducationLevel(educations)));
    ...
    private Map<HHUtils.EducationType, Collection<Education>> splitEducations(Set<Education> educations) {
        ...
        for (Education education : educations) {
            switch (education.getLevel()) {
                case SECONDARY:
                    result.get(HHUtils.EducationType.ELEMENTARY).add(education);
                    break;
                case SPECIAL_SECONDARY:
                case HIGHER:
                case UNFINISHED_HIGHER:
                    result.get(HHUtils.EducationType.PRIMARY).add(education);
            }
        }
        return result;
    }
...
//        jsonCompany.addProperty("id", String.valueOf(company.getHHId()));
//        experienceJson.add("company", jsonCompany);
    ...
    private JsonObject serializePhone(String phone) {
        ...
        phoneContactJson.addProperty("comment", "some comment");
